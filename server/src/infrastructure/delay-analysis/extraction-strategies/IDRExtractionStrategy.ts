...
export class IDRExtractionStrategy implements IDocumentExtractionStrategy {
  ...
  buildExtractionPrompt(context: DocumentExtractionContext): ExtractionStrategyResult {
    const toolBased = context.toolBasedExtraction === true;
    const truncatedContent = context.documentContent.slice(0, 30000);
    const knowledgeBasePrompt = context.skipKnowledgeBase
      ? ''
      : this.knowledgePromptBuilder.buildPromptForDocumentType('idr', context.documentContent);
    ...
    // The narrative walk-through, timestamp duration calculation, "bounded_by_next_entry"
    // bounding, the sourceReference timestamp requirement, and the zero-events conditions are
    // stated in full exactly once: here, when this strategy runs standalone (AIDelayEventExtractor).
    // When embedded into a tool-based call (AIDelayEventExtractorWithTools), the matching
    // IDRToolExtractionSystemPromptStrategy system message already states them, so this prompt
    // only points back to that single source instead of restating it in different wording.
    const narrativeRulesPointer = 'Narrative walk-through, timestamp duration calculation, ' +
      '"bounded_by_next_entry" bounding, and the sourceReference timestamp requirement are specified ' +
      'once in the system instructions above — apply them exactly as written there. Nothing below ' +
      'restates or overrides them.';

    const durationEstimationSection = toolBased
      ? `- DURATION ESTIMATION (REQUIRED): You MUST provide impactDurationHours for EVERY delay event.
  (${narrativeRulesPointer})`
      : `- DURATION ESTIMATION (REQUIRED): You MUST provide impactDurationHours for EVERY delay event.
  ...
  See the DIARY / NARRATIVE ANALYSIS section below for the full rules and worked examples for #2 and #3.`;
    ...
    const diaryNarrativeSection = toolBased
      ? `=============================================================================
DIARY / NARRATIVE ANALYSIS
=============================================================================

${narrativeRulesPointer} That includes reading every timestamped entry before concluding there are
no delays, and treating a "None"/blank summary field, a missing activity table, or absent CODE_CIE
tags as no evidence of absence — see NARRATIVE ANALYSIS in the system instructions.`
      : `=============================================================================
...
this is the one rule in this document that must never be skipped or shortened.
...
**REQUIRED CHECK BEFORE WRITING durationBasis: "estimated":** If the event's own diary entry has a
...
If that next entry is within a few hours, use "bounded_by_next_entry" instead of "estimated",
...
that is bounded by the next distinct entry's timestamp (bounded_by_next_entry), compute the
...
the next entry is too far away (more than a few hours) to plausibly bound the same delay.`;
    ...
    const zeroEventsPointer = 'The conditions that do — and do not — justify returning zero delay ' +
      'events are specified once in the system instructions\' NARRATIVE ANALYSIS section above ' +
      '(a "None"/blank summary field, a missing activity table, and absent CODE_CIE tags are never ' +
      'grounds to stop). Apply that rule as written; nothing below overrides it.';

    const responseFormatSection = toolBased
      ? `Return the JSON object's workActivities and delayEvents arrays exactly as specified in the
system instructions' output-format section above; the API response schema enforces that shape.`
...
${renderDelayEventOutputFormatBlock({
  ...
  workActivitiesExample: '[\n    {"activityId": "2-W-0471", "description": "Stage 1 WM: Excavate Services", "comments": "WM STA 7+00 to 21+50"}\n  ]',
    ...
    const zeroEventsSection = toolBased
      ? `WHEN TO RETURN ZERO DELAY EVENTS: ${zeroEventsPointer}`
      : `WHEN TO RETURN ZERO DELAY EVENTS:
...
excluded per the knowledge base.`;
    ...
    const documentContentSection = toolBased
      ? '\n\n(The full document content was already provided above in the user message; nothing further follows here.)'
      : `\n\nDocument content:\n${truncatedContent}`;
