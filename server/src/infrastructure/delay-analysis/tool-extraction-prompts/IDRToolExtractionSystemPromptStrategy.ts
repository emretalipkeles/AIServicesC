...
3. If the event starts at a stated clock time but the narrative never returns to it — the delay is only closed by the NEXT timestamped entry, which describes a DIFFERENT event or observation: treat that next timestamp as the point the delay ended, and CALCULATE the difference → durationBasis: "bounded_by_next_entry". Only use this when the gap to the next entry is a plausible delay length (a few hours at most) — do not bound against an entry many hours or a full shift later; when the gap is implausible, fall back to an estimate instead. Whenever you use this basis, ALSO set fallbackEstimateHours to your independent estimate of the delay's real length based on its nature — the server validates your window and, if it rejects it as implausible, falls back to fallbackEstimateHours rather than the rejected gap, so make it a genuine standalone estimate, not a copy of the window span.
...
- "1300 slip-form curb machine broke, had to stop" ... next entry "1530 exposed water main found" (a different, unrelated event) → the curb event is bounded by the 15:30 entry: impactDurationHours: 2.5, impactedWindowStart: "13:00", impactedWindowEnd: "15:30", durationBasis: "bounded_by_next_entry", fallbackEstimateHours: 1.5 (your independent estimate of how long a broken slip-form machine plausibly delays curb work, in case the server rejects the 2.5h window)
- "0745 concrete truck rejected for high slump" ... next entry is "1400" (6.25h later, a different bid item) — too far to plausibly bound → do NOT use bounded_by_next_entry at all; go straight to durationBasis: "estimated" with impactDurationHours set to your real independent estimate (e.g. 0.75h for a typical redispatch/retest wait), not the 6.25h gap
