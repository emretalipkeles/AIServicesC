...
- **Delay Analysis Feature**: AI-powered construction delay analysis. Processes project documents (IDRs, NCRs, Field Memos) to extract delay events and match them to CPM schedule activities. Includes project management APIs, real-time SSE progress reporting, run-based AI token usage tracking, and per-run cost display in USD shown in the UI after each operation completes (both for schedule uploads and delay analysis runs).
