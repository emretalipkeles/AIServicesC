...
**GPT-5.2 Pricing (official, as of Feb 2026):** Input: $1.750 / 1M tokens, Cached input: $0.175 / 1M tokens, Output: $14.000 / 1M tokens. Per-1K rates: input $0.00175/1K, output $0.014/1K. All cost calculations in `AITokenUsage.calculateCost()` must use these rates. The database is hosted on AWS — connection string is in the `AWS_DATABASE_URL` secret.
...
- **Date-Filtered Analysis**: AI analysis can be filtered by month/year to process only documents from a specific period, reducing costs and focusing analysis on relevant timeframes. Filters are in the Delay Events tab UI. **Re-run safety**: When analysis is re-run for a period, existing delay events from successfully re-processed documents are deleted before saving new results, preventing duplicate accumulation. Events from documents that fail extraction are preserved. The cleanup runs after extraction and deduplication but before saving new events.
...
- **IDR Match Enforcement Policy**: Domain-layer business rule (`IIDRMatchEnforcementPolicy` interface + `IDRMatchEnforcementPolicy` implementation in `server/src/domain/delay-analysis/config/`) that ensures BOTH extraction paths enforce the same strict IDR-first matching rule. Validates pre-matched activity IDs against the IDR work activities list, rejects non-IDR matches, and enforces the 90% confidence floor. Used as a safety net in `RunAnalysisCommandHandler` for the tool-based extraction path (Path 2) — if the AI pre-matched to an activity not in the IDR list, the pre-match is stripped so the event flows through the strict `AIActivityMatcher` (Path 1). Follows DRY (single rule definition), DIP (depends on abstraction), and Clean Architecture (domain rule in domain layer). Injected via composition root in `bootstrap.ts`.
...
- **Contractor Delay Training Guide Knowledge Base**: A comprehensive 22-page training guide integrated into all AI extraction prompts. Lives in domain layer as `IDelayKnowledgeBase` interface + `ContractorDelayTrainingGuide` value object (`server/src/domain/delay-analysis/config/ContractorDelayTrainingGuide.ts`). Contains: delay definition with contract basis (Section 1-08.4, 1-08.8), 5 delay categories with 22+ specific indicators, exclusions list (DSCs, owner-directed suspensions, etc.), 8-entry decision framework (if-yes/if-no logic), 17 worked examples (delays, non-delays, gray areas), 5 gray area scenarios, 8 common pitfalls, and a quick-reference cheat sheet. The `DelayKnowledgePromptBuilder` in infrastructure layer formats this into document-type-specific AI prompts (IDRs get full guide including worked examples; NCRs get core sections only). **Deduplication**: In the tool-based extraction path (`AIDelayEventExtractorWithTools`), the KB is injected ONCE in the system prompt only; the strategy user prompt skips it via `skipKnowledgeBase: true` on `DocumentExtractionContext` to avoid sending ~7,440 tokens twice. The non-tool extraction path (`AIDelayEventExtractor`) still gets the KB in the strategy prompt (default behavior). The AI Chat Assistant uses a separate path (`AgentExecutor` → `DelayEventsAgentContextProvider`) and is unaffected. Supersedes the old `DelayDefinitionConfig.ts` (deprecated, retained for backward compatibility).
...
- **Single-Document Analysis**: Per-document "Run AI Analysis" button in the Documents tab. Uses `RunSingleDocumentAnalysisCommandHandler` (CQRS command, separate from the full `RunAnalysisCommandHandler` per SRP). SSE endpoint at `GET /api/delay-analysis/projects/:projectId/documents/:documentId/analyze/stream`. Deletes existing events for the document before re-extracting, then runs extraction + matching for just that document. Enforces same IDR-first matching rules, IDR enforcement policy, and date validation as the full analysis. Documents tab also includes a text search filter for filtering by filename. Per-doc analysis buttons are disabled when a full analysis run is in progress (uses `isAnalyzing` from upload context). Zod validation at the boundary via `runSingleDocAnalysisParamsSchema`.
- **Analysis Progress Persistence**: Analysis run status survives page refresh. Uses in-memory `IAnalysisRunTracker` (domain interface) + `InMemoryAnalysisRunTracker` (infrastructure, singleton) to track run lifecycle (start → progress → complete/fail). `TrackingProgressReporter` decorator wraps the SSE progress reporter to simultaneously update the tracker. `GetAnalysisRunStatusQueryHandler` (CQRS query) exposed via `GET /api/delay-analysis/projects/:projectId/analysis/status`. Frontend polls every 2s on mount if a run is active, handles completed/failed terminal states on reconnect. Completed runs auto-cleaned from memory after 5 minutes (YAGNI: no DB table for ephemeral status).
